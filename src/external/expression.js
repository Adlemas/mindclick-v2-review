...
const nf_expression = (terms_length, min, max) => Expression(terms_length, min, max, nf_term);

const nf_term = (num, isAdditive, max_num_len, isHard, isRepeat, max) => check(num, isAdditive, max_num_len, NF, isHard, isRepeat, max);

const lf_expression = (terms_length, min, max) => Expression(terms_length, min, max, lf_term);

const lf_term = (num, isAdditive, max_num_len) => check(num, isAdditive, max_num_len, LF);

const bf_expression = (terms_length, min, max, isBiggerMax) => Expression(terms_length, min, max, bf_term, isBiggerMax);

const bf_term = (num, isAdditive, max_num_len, isHard) => check(num, isAdditive, max_num_len, BF, isHard);

const ff_expression = (terms_length, min, max) => Expression(terms_length, min, max, ff_term, true);

const ff_term = (num, isAdditive, max_num_len) => check(num, isAdditive, max_num_len, FF, true);

const sub_expression = (terms_length, min, max, sub_name) => Expression(
    terms_length,
    min,
    max,
    SUB_TERMS[sub_name]()[0],
    true,
);

const SUB_TERMS = {
    "+1=+5-4": () => [
        (num, isAdditive, max_num_len, isHard, isRepeat, max, min) => check(
            num,
            isAdditive,
            max_num_len,
            p1p5m4,
